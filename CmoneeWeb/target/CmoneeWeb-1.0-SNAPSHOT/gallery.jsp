<%-- 
    Document   : gallery
    Created on : Jun 4, 2020, 4:11:29 PM
    Author     : David
--%>
  ...
  <body class="size-1140">
    ...
    <!-- MAIN -->
      ...
      <article>
        ...
        <h2 class="text-size-50 text-center">Các bộ phim</h2>
        <h2 id="start"></h2>
        <hr class="break-small background-primary margin-top-bottom-20 break-center"> 
        <div class="tabs background-primary-hightlight">
            <div class="tab-item tab-active">
              <a class="tab-label active-btn">Đang chiếu</a>
              <div class="tab-content lightbox-gallery">
                  
                  <!--list đang chiếu-->
            <%   for (int i=0; i<list.size(); i++){
                    if(list.get(i).getIsPublished()==1){ %>
                <div class="s-12 m-6 l-3">
                  <a class="image-with-hover-overlay image-hover-zoom margin-left-right-10 margin-top-bottom-10" href="<%=DBUtil.fullPath("MVInfoController?MovieId=")+list.get(i).getId()%>">
                    <div class="image-hover-overlay background-primary"> 
                      <div class="image-hover-overlay-content text-center padding-2x">
                        <h2 class="text-thin text-strong"><%=list.get(i).getName()%></h2> 
                        <h3 class="text-thin"><%=list.get(i).getDirector()%> /<br> <%=MVInfoDAO.getCategory(list.get(i).getCategoryId())%> / <%=list.get(i).getNation()%></h3>
                        <p>Nội dung phim</p> 
                      </div> 
                    </div> 
                    <img src="<%=list.get(i).getPoster()%>" alt="" title="Portfolio Image 6" />
                  </a>  
                </div>
                <%}}%>
            ...
            <div class="tab-item">
              <a class="tab-label">Sắp chiếu</a>
              <div class="tab-content lightbox-gallery">
                  
                  <!--list sắp chiếu-->
                 <%   for (int i=0; i<list.size(); i++){
                    if(list.get(i).getIsPublished()==0){ %>
                <div class="s-12 m-6 l-3">
                  <a class="image-with-hover-overlay image-hover-zoom margin-left-right-10 margin-top-bottom-10" href="<%=DBUtil.fullPath("MVInfoController?MovieId=")+list.get(i).getId()%>">
                    <div class="image-hover-overlay background-primary"> 
                      <div class="image-hover-overlay-content text-center padding-2x">
